...
export default function CardGridWithPagination() {
    ...
    const isHugeTablet = useMediaQuery(theme.breakpoints.between('md', 'lg'));
    const isDesktop = useMediaQuery(theme.breakpoints.up('lg'));

    // Dynamic cards per page
    let cardsPerPage = 1;
    if (isTablet) cardsPerPage = 4;
    if (isHugeTablet) cardsPerPage = 3;
    ...
    return (
        <div className="w-full h-full  py-5 flex flex-col">
            <div className="hidden justify-between gap-4 mb-4 md:flex">
            ...
            <div
                className={`grid gap-4 mb-4 grid-cols-1 sm:grid-cols-2 px-4 overflow-scroll md:hidden `}
            >                {visibleCards.map((card) => (
                <CustomCard key={card.id} title={card.title} img={card.img} />
            ))}
            </div>
